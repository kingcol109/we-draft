  ...
  return (
    <div style={{ padding: "60px 140px", background: "#fafafa" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 50 }}>
        <div style={{ fontSize: 34, fontWeight: 900, color: SITE_BLUE, marginBottom: 6 }}>
          {(user?.displayName || user?.email?.split("@")[0] || "User")}'s Whiteboard
        </div>
        <div style={{ fontSize: 14, color: "#666", marginBottom: 18, textAlign: "center" }}>
        ...
        <select
          ...
          style={{
            padding: "10px 18px", fontSize: 18, fontWeight: 600,
            border: `2px solid ${SITE_GOLD}`, borderRadius: 6,
            color: SITE_BLUE, background: "#fff", cursor: "pointer"
          }}
        >
          {DRAFT_CLASSES.map(year => (
            <option key={year} value={year}>
              {year === "2026" ? "2026 (Archive)" : year}
            </option>
          ))}
        </select>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 6, fontWeight: "bold", cursor: "pointer", fontSize: 15
          }}
        >
      ...
      <button
        ...
        style={{
          marginTop: 12, background: "#fff", color: SITE_BLUE,
          padding: "8px 22px", border: `2px solid ${SITE_BLUE}`,
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <DragOverlay>
          {activeId ? (
            <div style={{ padding: "6px 10px", border: "1px solid #ccc", background: "#fff", fontSize: 14, boxShadow: "0 6px 18px rgba(0,0,0,0.2)", borderRadius: 4, cursor: "grabbing" }}>
              {(() => {
                const player = allPlayers.find(p => p.id === activeId);
                if (!player) return "";
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 50, background: "#fafafa", display: "grid", gridTemplateColumns: `120px repeat(${POSITIONS.length}, 1fr)`, paddingBottom: 20, marginBottom: 25, borderBottom: `3px solid ${SITE_GOLD}` }}>
          <div></div>
          {POSITIONS.map(pos => (
            <div key={pos} style={{ textAlign: "center", fontWeight: "900", fontSize: 24, color: SITE_BLUE }}>{pos}</div>
          ))}
        </div>

        {/* ROUNDS */}
        {ROUNDS.map(round => (
          <div key={round} style={{ display: "grid", gridTemplateColumns: `120px repeat(${POSITIONS.length}, 1fr)`, marginBottom: 40 }}>
            <div style={{ writingMode: "vertical-rl", transform: "rotate(180deg)", fontSize: 26, fontWeight: "900", color: SITE_BLUE, textAlign: "center" }}>
              {round}
            </div>
            {POSITIONS.map(pos => (
              <PositionColumn key={`${round}-${pos}`} round={round} pos={pos} playerIds={board[round][pos]} players={allPlayers} />
            ))}
          </div>
        ))}
        ...
        <div ref={playerBankRef} style={{ marginTop: 100 }}>
          <div style={{ textAlign: "center", fontSize: 26, fontWeight: "900", color: SITE_BLUE, marginBottom: 35 }}>
            PLAYER BANK
          </div>
          <div style={{ display: "grid", gridTemplateColumns: `120px repeat(${POSITIONS.length}, 1fr)` }}>
            <div></div>
            {POSITIONS.map(pos => {
              ...
              return <BankColumn key={pos} pos={pos} players={playersForPos} />;
            })}
          </div>
        </div>
      </DndContext>
...
function PositionColumn({ round, pos, playerIds, players }) {
  const { setNodeRef } = useDroppable({ id: `${round}::${pos}` });
  return (
    <div ref={setNodeRef} style={{ minHeight: 190, background: "#fff", border: `2px solid ${SITE_GOLD}`, padding: 10 }}>
      <SortableContext items={playerIds} strategy={verticalListSortingStrategy}>
        {playerIds.map(id => {
          const player = players.find(p => p.id === id);
          if (!player) return null;
          return <DraggablePlayer key={id} id={id} player={player} />;
        })}
      </SortableContext>
    </div>
  );
...
function BankColumn({ pos, players }) {
  const { setNodeRef } = useDroppable({ id: `bank::${pos}` });
  return (
    <div ref={setNodeRef} style={{ minHeight: 190, background: "#fff", border: `2px solid ${SITE_GOLD}`, padding: 10 }}>
      <SortableContext items={players.map(p => p.id)} strategy={verticalListSortingStrategy}>
        {players.map(player => (
          <DraggablePlayer key={player.id} id={player.id} player={player} />
        ))}
      </SortableContext>
    </div>
  );
...
function DraggablePlayer({ id, player }) {
  ...
  const POS_COLORS = {
    ...
    OL: "#FBC02D", EDGE: "#8E24AA", DL: "#0097A7", LB: "#0D47A1", DB: "#EC407A"
  };
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <a
        ...
        style={{ textDecoration: "none", color: SITE_BLUE, fontWeight: 800, fontSize: 14, lineHeight: "16px", display: "block", textAlign: "center" }}
        onClick={(e) => e.stopPropagation()}
      >
        {`${(player.First || "").charAt(0)}. ${player.Last || ""}`}
      </a>
      <div style={{ fontSize: 9, fontStyle: "italic", opacity: 0.6, marginTop: 2, lineHeight: "11px", textAlign: "center" }}>
        {player.School || ""}
      </div>
    </div>
  );
